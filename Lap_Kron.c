                 ...
                 double _Complex *VyH, double _Complex *VzH, double _Complex *vec, double *diag, double _Complex *out)
{
    ...
    for (int k = 0; k < Nz; k++) {
        cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Nx, Ny, Ny, 
                    &alpha, vec + k*NxNy, Nx, VyH, Ny, &beta, vecTVy, Nx);

        cblas_zgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, Nx, Ny, Nx, 
                    &alpha, Vx, Nx, vecTVy, Nx, &beta, VxtvecTVy + k*NxNy, Nx);
    }
    ...
    for (int k = 0; k < Nz; k++) {
        cblas_zgemm(CblasColMajor, CblasNoTrans, CblasTrans, Nx, Ny, Ny, 
                    &alpha, P + k*NxNy, Nx, Vy, Ny, &beta, PTVyt, Nx);

        cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Nx, Ny, Nx, 
                    &alpha, Vx, Nx, PTVyt, Nx, &beta, VxPTVyt + k*NxNy, Nx);
    }
                 ...
                 double _Complex *VyH, double _Complex *VzH, double _Complex *vec, int ncol, double *diag, double _Complex *out)
{
    ...
    int Nd = Nx * Ny * Nz;
    int len = Nd * ncol;
    double _Complex *temp = (double _Complex *) malloc(sizeof(double _Complex) * len);
    
    double _Complex *VxTvec, *VxTvecVy, *P, *DP, *VxDP, *VxDpVyT, *res;
    double _Complex *FormVxTvec, *FormVxTvecVy, *FormDP, *FormVxDP, *FormVxDpVyT;
    double _Complex alpha = 1, beta = 0;
    ...
    // P = Lambda .* (Vz' x Vy' x Vx') * vec
    cblas_zgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, Nx, Ny*Nz*ncol, Nx, 
                &alpha, Vx, Nx, vec, Nx, &beta, VxTvec, Nx);

    f1Tof2(Nx, Ny, Nz, ncol, VxTvec, FormVxTvec, sizeof(double _Complex));

    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Nx*Nz*ncol, Ny, Ny, 
                &alpha, FormVxTvec, Nx*Nz*ncol, VyH, Ny, &beta, VxTvecVy, Nx*Nz*ncol);


    f2Tof4(Nx, Ny, Nz, ncol, VxTvecVy, FormVxTvecVy, sizeof(double _Complex));

    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, NxNy*ncol, Nz, Nz, 
                &alpha, FormVxTvecVy, NxNy*ncol, VzH, Nz, &beta, P, NxNy*ncol);
    
    // apply diagonal term
    double _Complex *P_ = P, *DP_ = DP;
    for (int nz = 0; nz < Nz; nz++) {
        double *diag_ = diag + nz*Nx*Ny;
        for (int n = 0; n < ncol; n++) {
            for (int i = 0; i < Nx*Ny; i++) {
    ...
    // out = (Vz x Vy x Vx) * P
    f4Tof1(Nx, Ny, Nz, ncol, DP, FormDP, sizeof(double _Complex));

    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Nx, Ny*Nz*ncol, Nx, 
                &alpha, Vx, Nx, FormDP, Nx, &beta, VxDP, Nx);

    f1Tof2(Nx, Ny, Nz, ncol, VxDP, FormVxDP, sizeof(double _Complex));

    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasTrans, Nx*Nz*ncol, Ny, Ny, 
                &alpha, FormVxDP, Nx*Nz*ncol, Vy, Ny, &beta, VxDpVyT, Nx*Nz*ncol);

    f2Tof4(Nx, Ny, Nz, ncol, VxDpVyT, FormVxDpVyT, sizeof(double _Complex));

    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasTrans, NxNy*ncol, Nz, Nz, 
                &alpha, FormVxDpVyT, NxNy*ncol, Vz, Nz, &beta, res, NxNy*ncol);

    f4Tof1(Nx, Ny, Nz, ncol, res, out, sizeof(double _Complex));
