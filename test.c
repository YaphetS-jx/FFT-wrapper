...
{
    int Nd = Nx * Ny * Nz;
    double _Complex *X = (double _Complex*) malloc(sizeof(double _Complex) * Nd*ncol);
    double _Complex *LapX = (double _Complex*) malloc(sizeof(double _Complex) * Nd*ncol);
    double _Complex *LapX_gpu = (double _Complex*) malloc(sizeof(double _Complex) * Nd*ncol);
    double _Complex *VyH = (double _Complex*) malloc(sizeof(double _Complex) * Ny*Ny);
    double _Complex *VzH = (double _Complex*) malloc(sizeof(double _Complex) * Nz*Nz);
    for (int i = 0; i < Ny*Ny; i++) VyH[i] = conj(Vy[i]);
    for (int i = 0; i < Nz*Nz; i++) VzH[i] = conj(Vz[i]);
    rand_vec((double *)X, 2*Nd*ncol);

    // gpu variables
    cudaError_t cuE; cublasStatus_t cubSt;
    cuDoubleComplex *d_X, *d_LapX, *d_VyH, *d_VzH;
    cuE = cudaMalloc((void **) &d_X, sizeof(cuDoubleComplex) * Nd*ncol); assert(cudaSuccess == cuE);
    cuE = cudaMalloc((void **) &d_LapX, sizeof(cuDoubleComplex) * Nd*ncol); assert(cudaSuccess == cuE);
    cuE = cudaMalloc((void **) &d_VyH, sizeof(cuDoubleComplex) * Ny*Ny); assert(cudaSuccess == cuE);
    cuE = cudaMalloc((void **) &d_VzH, sizeof(cuDoubleComplex) * Nz*Nz); assert(cudaSuccess == cuE);
    ...
    // for (int i = 0; i < ncol; i++) {
        // Lap_Kron_complex(Nx, Ny, Nz, Vx, Vy, Vz, VyH, VzH, X+i*Nd, diag, LapX+i*Nd);
    // }

    Lap_Kron_multicol_complex(Nx, Ny, Nz, Vx, Vy, Vz, VyH, VzH, X, ncol, diag, LapX);

    CUDA_Lap_Kron_multicol_complex(Nx, Ny, Nz, d_Vx, d_Vy, d_Vz, d_VyH, d_VzH, d_X, ncol, d_diag, d_LapX);

    cubSt = cublasGetVector(Nd*ncol, sizeof(cuDoubleComplex), d_LapX, 1, LapX_gpu, 1); assert(CUBLAS_STATUS_SUCCESS == cubSt);
    
    printf("err between CPU and GPU for multiple column way complex: %.2e\n", err((double *)LapX, (double *)LapX_gpu, 2*Nd*ncol));    
                ...
                double _Complex *Vx, double _Complex *Vy, double _Complex *Vz, double *diag, int reps)
{
    ...
    printf("kron_multiple_col_complex_CPU %d times takes on average : %.2f ms\n", reps, t_cpu/reps);
